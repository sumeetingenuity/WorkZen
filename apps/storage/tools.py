...
async def search_documents(
    query: str = "", file_type: str = "", limit: int = 20
) -> dict:
    from asgiref.sync import sync_to_async
    from apps.storage.models import StoredDocument
...
async def get_last_document(user_id: str = "") -> dict:
    ...
    def _get():
        if user_id:
            # Get last document for specific user
            return (
                StoredDocument.objects.filter(user_id=user_id)
                .order_by("-created_at")
                .first()
            )
        else:
            # Get last document from any user
            return StoredDocument.objects.order_by("-created_at").first()

    doc = await sync_to_async(_get)()
    if not doc:
        return {"error": "No stored documents found."}
    return {
        "id": str(doc.id),
